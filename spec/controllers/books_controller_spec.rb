RSpec.describe BooksController, type: :controller do
  describe '.index' do
    describe 'with general categories' do
      before { get :index }
    ...
    describe 'with the category' do
      let(:book) { create(:book)}
      before { get :index, params: {category: book.category } }
      ...
      it { expect(assigns(:books)).to be_kind_of(ActiveRecord::Relation) }
      it { expect(assigns(:current_filter)).to be_kind_of(String) }
      it { expect(assigns(:category)).to be_kind_of(String) }
    end
  end
